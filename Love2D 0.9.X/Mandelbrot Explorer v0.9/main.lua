...
local effect=love.graphics.newShader(love.filesystem.read'Mandelbrot.glsl')
local send=effect.send
send(effect,"maxi",1e3)
send(effect,"vs",vs)


local thumb={pos={5,5},view={133,100}}
local viewsize={pos={0,0},view={vs[1],vs[2]}}

local default={pos={-2,-1.0625},view={3,2.125}}
local window={pos={-2,-1.0625},view={3,2.125}}
...
function love.update(t)
	...
	window.pos={window.pos[1]+(window.view[1]-newview[1])*mx/vs[1]+newview[1]*scroll[1]*scrollsec*t,window.pos[2]+(window.view[2]-newview[2])*my/vs[2]-newview[2]*scroll[2]*scrollsec*t}
	window.view=newview

	if isBtn'l' then
		if not grab then
			grab={window.pos[1]+mx/vs[1]*window.view[1],window.pos[2]+my/vs[2]*window.view[2]}
		end
		window.pos[1]=grab[1]-mx/vs[1]*window.view[1]
		window.pos[2]=grab[2]-my/vs[2]*window.view[2]
	elseif grab then
		window.pos[1]=grab[1]-mx/vs[1]*window.view[1]
		window.pos[2]=grab[2]-my/vs[2]*window.view[2]
		grab=nil
	end
	setTitle("Mandelbrot Explorer v"..version.." - "..fps().." FPS")
end
...
function love.resize(w,h)
	...
	send(effect,"vs",vs)
	viewsize.view={w,h}
	thumb.view[1]=thumb.view[2]*w/h
	default.view[1]=default.view[2]*w/h
	window.view[1]=window.view[2]*w/h
end
...
function love.mousepressed(x,y,button)
	if button=="wu" then
		...
		window.pos={window.pos[1]+(window.view[1]-newview[1])*x/vs[1],window.pos[2]+(window.view[2]-newview[2])*y/vs[2]}
		window.view=newview

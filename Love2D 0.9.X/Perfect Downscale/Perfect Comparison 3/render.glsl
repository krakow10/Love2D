...
int fuckinghell(int a,int b)//max(a,b) is ambiguous :::::::::::::::::))))))))))))))))))))))))
{
	return a>b?a:b;
}
...
vec4 effect(vec4 _0,Image _1,vec2 _2,vec2 iPixel){
	...
	for(int x=fuckinghell(-1,ix_min.x);x<=min(sizei.x,ix_max.x);++x){
		int yinit,ystop;
		...
		for(int y=fuckinghell(int(-1),yinit);y<=min(sizei.y,ystop);++y){
			//AVERT YOUR EYES
			number Area=0;

...
vec4 effect(vec4 _0,sampler2D image,vec2 tex,vec2 _3){
	...
	for(int i=1;i<ceil(violence-0.5);++i){
		...
		colour+=(thiserf-lasterf)*(texture2D(image,tex-vec2(i,0)/size)+texture2D(image,tex+vec2(i,0)/size));
		lasterf=thiserf;
	}
	return (colour+(erf(scale)-lasterf)*(texture2D(image,tex-vec2(ceil(violence),0)/size)+texture2D(image,tex+vec2(ceil(violence),0)/size)))/(2*erf(scale));
}

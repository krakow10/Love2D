...
vec4 getColour(vec3 Origin,vec3 Direction){
	vec2 Du=(Origin.xy-Origin.z/Direction.z*Direction.xy)/vs;
	if(Direction.z>=0.0&&(Du.x<0.0||Du.x>1.0||Du.y<0.0||Du.y>1.0))
		return Texel(Sky,0.25*Direction.xy/(Direction.z-1)*vec2(SkySize.y/SkySize.x,1)+vec2(0.5));
	else if(Direction.z<0.0)
		return Texel(Tile,Du*vec2(TileSize.y/TileSize.x,1));
	else
		return vec4(0,0,0,1);
}
...
vec4 fresnel(vec3 P,vec3 I,vec3 N,number eta)
{
	...
	if(k>=0.0)
		T=normalize(eta*I-(eta*dNI+sqrt(k))*N);//Swag

	number mag=dot(N,T)/dNI;//Magnification
	vec2 cd=vec2(1.0+mag*eta,mag+eta);//Common divisor
	vec2 r=vec2(1.0-mag*eta,mag-eta)/cd;//Reflected (S,P)
	vec2 t=vec2(2.0)/cd;//Transmitted (S,P)
...
vec4 effect(vec4 _0,Image hMap,vec2 percent,vec2 ipixel)
{
	...
	vec3 Eye=vec3(View[1]/2.0,depth);
	vec3 Point=vec3(pixel,height(Texel(hMap,percent)));
	vec3 dx=vec3(10,0,height(Texel(hMap,percent+vec2(1,0)/vs))-height(Texel(hMap,percent-vec2(1,0)/vs)));
	vec3 dy=vec3(0,10,height(Texel(hMap,percent+vec2(0,1)/vs))-height(Texel(hMap,percent-vec2(0,1)/vs)));
	return fresnel(Point,normalize(Point-Eye),normalize(cross(dx,dy)),0.1);//1.333/1.000277
}

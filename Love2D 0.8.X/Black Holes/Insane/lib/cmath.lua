...
local complex_mt={
	...
	__mod=function(c1,c2)
		...
		local rsq=r2^2+i2^2
		--a%b=a-floor(a/b)*b
		local m3=floor(sqrt(((r1*r2+i1*i2)/rsq)^2+((r2*i1-r1*i2)/rsq)^2))
		return complex(r1-r2*m3,i1-i2*m3)
	end,
...
function cmath.ceil(c)
	local r,i=re(c),im(c)
	local len=sqrt(r^2+i^2)
	local mul=ceil(len)/len
	return complex(r*mul,i*mul)
end
function cmath.floor(c)
	local r,i=re(c),im(c)
	local len=sqrt(r^2+i^2)
	local mul=floor(len)/len
	return complex(r*mul,i*mul)
end
...
--Inverse trig functions (Accept real numbers plz)
function cmath.asin(c)
	return pi/2+i*ln(c+(c^2-1)^0.5)
end
function cmath.acos(c)
	return -i*ln(c+(c^2-1)^0.5)
end
...
function cmath.atan2(c2,c1)--y,x
	...
	return complex(arg(c3/c4^0.5),-log(cmath.abs(c3)/cmath.abs(c4)^0.5))
end

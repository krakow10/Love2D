...
vec4 fog(vec3 x)
{
	return vec4(snoise(x+vec3(0,1,1)),snoise(x+vec3(1,0,1)),snoise(x+vec3(1,1,0)),snoise(x));
}
...
vec4 effect(vec4 _0,Image _1,vec2 _3, vec2 pixel){
	...
	for(s=0;p>0.005;s+=sdx){
		...
		vec4 c1=fog(point);
		number pNext=p*pow(c1.a,sdx);
		colour+=c1.rgb*(p-pNext);
		p=pNext;
		if(omg||point.y<0)
			break;
	}
	if(dir.y<0&&s>=-pos.y/dir.y){
		...
		return vec4(mix(colour,tColour.rgb,p),1-p*(1-tColour.a));
	}else
		return vec4(colour,1-p);
}
